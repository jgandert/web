...
const paper = function(self, undefined) {
    ...
    const Base = new function() {
        const hidden = /^(statics|enumerable|beans|preserve)$/,
            ...
            each = function(obj, iter, bind) {
                if (obj) {
                    const desc = describe(obj, "length");
        ...
        function inject(dest, src, enumerable, beans, preserve) {
            ...
            function field(name, val) {
                ...
                let isFunc = typeof val === "function",
                    res = val,
    ...
    Base.inject({
        ...
        statics: {
            ...
            readSupported: function(list, dest) {
                let source = this.getSource(list),
                    that = this,
                    read = false;
                if (source) {
    ...
    var Numerical = new function() {
        ...
        return {
            ...
            solveCubic: function(a, b, c, d, roots, min, max) {
                let f = getNormalizationFactor(abs(a), abs(b), abs(c), abs(d)),
                    x, b1, c2, qd, q;
                if (f) {
                    a *= f;
                    b *= f;
    ...
    var Rectangle = Base.extend({
        ...
        initialize: function Rectangle(arg0, arg1, arg2, arg3) {
            ...
            if (read === undefined) {
                let frm = Point.readNamed(args, "from"),
                    next = Base.peek(args),
                    x = frm.x,
                    y = frm.y,
                    width,
    ...
    },
    {
        ...
        getBounds: function(matrix, options) {
            ...
            {opts.cacheItem = this;}
            const rect = this._getCachedBounds(hasMatrix && matrix, opts).rect;
        ...
        _getCachedBounds: function(matrix, options, noInternal) {
            ...
            let internal = options.internal && !noInternal,
    ...
    new function() {
        ...
        _hitTest: function(point, options, parentViewMatrix) {
            ...
            let checkSelf = !(options.guides && !this._guide
        ...
        transform: function(matrix, _applyRecursively, _setApplyMatrix) {
            ...
            if (decomp && decomp.skewing.isZero() && decomp.rotation % 90 === 0) {
                ...
                this._bounds = bounds;
                const cached = bounds[this._getBoundsCacheKey(
                    this._boundsOptions || {})];
                if (cached) {
    ...
    new function() {
        ...
        statics: new function() {
            function createShape(type, point, size, radius, args) {
                const item = Base.create(Shape.prototype);
                item._type = type;
                item._size = size;
                item._radius = radius;
    ...
    var Raster = Item.extend({
        ...
        setSize: function(_size, _clear) {
            ...
            if (!size.equals(this._size)) {
                if (size.width > 0 && size.height > 0) {
                    const element = !_clear && this.getElement();
        ...
        getAverageColor: function(object) {
            ...
            ctx.restore();
            let pixels = ctx.getImageData(0.5, 0.5, Math.ceil(width),
    ...
    var SymbolItem = Item.extend({
        ...
        _hitTestSelf: function(point, options, viewMatrix) {
            const opts = options.extend({ all: false });
            const res = this._definition._item._hitTest(point, opts, viewMatrix);
            if (res)
            {res.item = this;}
            return res;
        },
    ...
    var PathItem = Item.extend({
        ...
        setPathData: function(data) {
            ...
            let parts = data && data.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/ig),
                coords,
                relative = false,
                previous,
                control,
        ...
        getNearestLocation: function() {
            let point = Point.read(arguments),
        ...
        compare: function(path) {
            let ok = false;
            if (path) {
                let paths1 = this._children || [this],
    ...
    var Path = PathItem.extend({
        ...
        setSegments: function(segments) {
            let fullySelected = this.isFullySelected(),
        ...
        divideAt: function(location) {
            let loc = this.getLocationAt(location),
                curve;
        ...
        splitAt: function(location) {
            ...
            if (index >= 0 && index < curves.length) {
                ...
                let segs = this.removeSegments(index, this._segments.length, true),
                    path;
                if (this._closed) {
        ...
        smooth: function(options) {
            ...
            if (/^(?:asymmetric|continuous)$/.test(type)) {
                let asymmetric = type === "asymmetric",
                    min = Math.min,
                    amount = to - from + 1,
                    n = amount - 1,
        ...
        _hitTestSelf: function(point, options, viewMatrix, strokeMatrix) {
            ...
            function checkSegmentStroke(segment) {
                const isJoin = closed || segment._index > 0
                ...
                if (!dontPaint && (hasFill || hasStroke)) {
                    ...
                    if (hasStroke) {
                        if (dashLength) {
                            if (!dontStart)
                            {ctx.beginPath();}
                            let flattener = new PathFlattener(this, 0.25, 32, false,
                                    strokeMatrix),
    ...
    new function() {
        ...
        statics: {
            ...
            _addSquareCap: function(segment, cap, radius, matrix, strokeMatrix,
                addPoint, isArea) {
                let point = segment._point.transform(matrix),
                    loc = segment.getLocation(),
            ...
            getHandleBounds: function(segments, closed, path, matrix, options) {
                let style = path.getStyle(),
                    ...
                    joinPadding;
                if (stroke) {
                    let strokeMatrix = path._getStrokeMatrix(matrix, options),
    ...
    PathItem.inject(new function() {
        ...
        function traceBoolean(path1, path2, operation, options) {
            ...
            {_path2.reverse();}
            let crossings = divideLocations(CurveLocation.expand(
        ...
        function splitBoolean(path1, path2, operation) {
            const _path1 = preparePath(path1),
    ...
    const TextItem = Item.extend({
        ...
        initialize: function TextItem(arg) {
            this._content = "";
            this._lines = [];
            const hasProps = arg && Base.isPlainObject(arg)
    ...
    var Style = Base.extend(new function() {
        ...
        getLeading: function getLeading() {
            let leading = getLeading.base.call(this),
    ...
    DomEvent.requestAnimationFrame = new function() {
        let nativeRequest = DomElement.getPrefixed(window, "requestAnimationFrame"),
            requested = false,
            callbacks = [],
            timer;
    ...
    new function() {
        ...
        return {
            ...
            _handleMouseEvent: function(type, event, point) {
                ...
                let inView = this.getBounds().contains(point),
    ...
    const Tool = PaperScopeItem.extend({
        ...
        _handleMouseEvent: function(type, event, point, mouse) {
            ...
            {type = "mousemove";}
            let move = mouse.move || mouse.drag,
    ...
    var CanvasProvider = Base.exports.CanvasProvider = {
        ...
        getContext: function(width, height, options) {
            const canvas = this.getCanvas(width, height, options);
    ...
    var BlendMode = new function() {
        ...
        const ctx = CanvasProvider.getContext(1, 1, { willReadFrequently: true });
        if (ctx) {
            Base.each(modes, function(func, mode) {
                let darken = mode === "darken",
                    ok = false;
                ctx.save();
                try {
    ...
    new function() {
        ...
        function exportSymbolItem(item, options) {
            let attrs = getTransform(item._matrix, true),
        ...
        function exportGradient(color) {
            ...
            if (!gradientNode) {
                let gradient = color.getGradient(),
        ...
        function exportDefinitions(node, options) {
            ...
            if (definitions) {
                ...
                for (const i in definitions.svgs) {
                    if (!defs) {
                        if (!svg) {
        ...
        function exportSVG(item, options, isRoot) {
            let exporter = exporters[item._class],
        ...
        Project.inject({
            exportSVG: function(options) {
                ...
                let node = SvgElement.create("svg", attrs, formatter),
                    parent = node;
    ...
    new function() {
        ...
        function importGradient(node, type) {
            let id = (getValue(node, "href", true) || "").substring(1),
        ...
        const importers = {
            ...
            image: function (node) {
                ...
                raster.on("load", function() {
                    ...
                    this.setSize(size);
                    const center = getPoint(node).add(size.divide(2));
